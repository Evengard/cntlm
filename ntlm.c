...
int ntlm_request(char **dst, char *hostname, char *domain) {
	char *buf;
	int dlen, hlen;
	...
	VAL(buf, uint32_t, 12) = U32LE(0xB207);
...
int ntlm_response(char **dst, char *challenge, char *username, char *password, char *hostname, char *domain) {
	char lm[24], nt[24];
	char *buf, *udomain, *uuser, *uhost;
	int dlen, ulen, hlen;

	ntlm_lm_password((char *)lm, password, MEM(challenge, char, 24));
	ntlm_nt_password((char *)nt, password, MEM(challenge, char, 24));
	...
	VAL(buf, uint16_t, 12) = U16LE(24);
	VAL(buf, uint16_t, 14) = U16LE(24);
	VAL(buf, uint32_t, 16) = U32LE(64+dlen+ulen+hlen);

	VAL(buf, uint16_t, 20) = U16LE(24);
	VAL(buf, uint16_t, 22) = U16LE(24);
	VAL(buf, uint32_t, 24) = U32LE(64+dlen+ulen+hlen+24);
	...
	VAL(buf, uint16_t, 56) = U16LE(64+dlen+ulen+hlen+24+24);
	...
	memcpy(MEM(buf, char, 64+dlen+ulen+hlen), lm, 24);
	memcpy(MEM(buf, char, 64+dlen+ulen+hlen+24), nt, 24);
	...
	*dst = buf;
	return 64+dlen+ulen+hlen+24+24;
}
